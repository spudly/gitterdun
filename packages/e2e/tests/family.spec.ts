...
test.describe('Family Management', () => {
  ...
  const registerAndLogin = async (page: any, suffix = '') => {
    const timestamp = Date.now() + Math.random();
    const username = `familyuser${timestamp}${suffix}`;
  ...
  test('should require family name to create family', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder*="family name"]')).toBeVisible();
  });
