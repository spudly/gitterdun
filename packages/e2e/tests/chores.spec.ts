...
test.describe('Chores Workflow', () => {
  ...
  const setupFamily = async (page: any) => {
  ...
  test('should assign chore to child', async ({ page }) => {
    ...
    // Verify assignment
    await expect(page.locator(`text=Assigned to: ${child.username}`)).toBeVisible();
  });
  ...
  test('should reject completed chore as parent', async ({ page }) => {
    ...
    // Reject the chore
    await page.locator(`text=${choreTitle}`).locator('..').locator('button:has-text("Reject")').click();
